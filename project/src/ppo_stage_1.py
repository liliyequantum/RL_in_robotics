...
def train(env, hyperparameters, actor_model, critic_model):
    ...
    # for trining, actor_model and critic_model should always be empty; for testing, they should have something strings 
    if actor_model != '' or critic_model != '':  # Don't train from scratch if user accidentally forgets actor/critic model
        print(
    ...
    critic_path = sorted(glob.glob(f'/root/catkin_ws/src/project/src/models/{agent.exp_id}/ppo_critic_*.pth'))
    actor_path  = sorted(glob.glob(f'/root/catkin_ws/src/project/src/models/{agent.exp_id}/ppo_actor_*.pth'))
    if hyperparameters['continue_train'] and critic_path != []:
...
def main(args):
    ...
    hyperparameters = {
        ...
        'render_every_i': 10,
        'log_dir': '',
        'exp_id': "complex_env_basic_reward_ppo",
        'continue_train': False
    }
    hyperparameters['log_dir'] = '/root/catkin_ws/src/project/src/summary/'+hyperparameters['exp_id']
    ...
    if args.mode == 'train':
        ...
        actor_path = sorted(glob.glob(f"/root/catkin_ws/src/project/src/models/{hyperparameters['exp_id']}/ppo_actor_*.pth"))
        args.actor_model = actor_path[-1]
