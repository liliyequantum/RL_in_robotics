...
class PPO:
    ...
    def __init__(self, policy_class, value_func, env, state_dim, action_dim, **hyperparameters):
        ...
        self.actor = policy_class(self.obs_dim, self.act_dim).to(device)  # input dim = obs_dim, output dim = act_dim, ALG STEP 1
    ...
    def learn(self, total_timesteps, past_action):
        ...
        while t_so_far < total_timesteps:  # ALG STEP 2
            ...
            f = open(self.record_dir + '/V_fun' + '.txt', 'a+')
            for i in value_func:
                f.write(str(i))
                f.write('\n')
            f.close()
            ...
            if i_so_far % self.save_freq == 0:
                ...
                torch.save(self.actor.state_dict(), f'/root/catkin_ws/src/project/src/models/{self.exp_id}/ppo_actor_update_{epoch}.pth')
                torch.save(self.critic.state_dict(), f'/root/catkin_ws/src/project/src/models/{self.exp_id}/ppo_critic_update_{epoch}.pth')
    ...
    def rollout(self, past_action, t_so_far):
        ...
        f = open(self.record_dir + '/ppo' + '.txt', 'a+')
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.log_dir = '/root/catkin_ws/src/project/src/summary/simple_env_basic_reward'
        # Miscellaneous parameters
        self.render = True  # If we should render during rollout
        self.render_every_i = 10  # Only render every n iterations
        self.save_freq = 5  # How often we save in number of iterations
        self.seed = None  # Sets the seed of our program, used for reproducibility of results
        self.exp_id = 'simple_env_basic_reward'
        ...
        ### create model folder
        exp_path = f'/root/catkin_ws/src/project/src/models/{self.exp_id}'
        makepath(exp_path)
        
        ### create record folder
        self.record_dir = f'/root/catkin_ws/src/project/src/record/{self.exp_id}'
        if not os.path.exists(self.record_dir):
            os.makedirs(self.record_dir)
    
        ####
        import yaml
        with open(f'/root/catkin_ws/src/project/src/models/{self.exp_id}' + '/config.yml', 'w') as outfile:
